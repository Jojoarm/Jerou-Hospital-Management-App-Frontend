...
import { BadgeCheck, BadgeInfo, Loader } from 'lucide-react';
...
const AppointmentReschedule = () => {
  ...
  const getDoctor = async () => {
    ...
    try {
      const { data } = await axios.get(
        `${backendUrl}/api/doctor/doctor-profile/${rescheduleDocId}`
      );
      if (data.success) {
        setIsLoading(false);
  ...
  const getAppointment = async () => {
    ...
    try {
      ...
      if (data.success) {
        setIsLoading(false);
        setRescheduleDocId(data.appointmentData.docId);
      } else {
        setIsLoading(false);
        toast.error(data.message);
      }
  ...
  return (
    <div className="mt-[150px] flex flex-col gap-10 p-5">
      ...
      <div className="flex flex-col gap-3">
        <h2 className="font-semibold text-2xl text-center text-indigo-950">
          Related Doctors
        </h2>
        <RelatedDoctors
          docId={rescheduleDocId}
